...
{
    return (uint32_t)(READ_BIT(UARTx->IRCR, 0xfU) >> 11U);
}
...
{
    return (uint32_t)(READ_BIT(UARTx->IRCR, 0x7ffU) >> 0U);
}
...
{
    return (uint32_t)(READ_BIT(UARTx->RXBUF, 0xffU) >> 0U);
}
...
{
    MODIFY_REG(UARTx->TXBUF, (0xffU << 0U), (data << 0U));
}
