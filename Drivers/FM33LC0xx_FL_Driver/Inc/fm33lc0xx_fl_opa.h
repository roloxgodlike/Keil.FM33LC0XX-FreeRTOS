...
#define    FL_OPA_MODE_STANDLONE                                  (0x0U << OPA_CR_MOD_Pos)
...
{
    return (uint32_t)(READ_BIT(OPAx->CALR, 0x1fU) >> 16U);
}
...
{
    return (uint32_t)(READ_BIT(OPAx->CALR, 0x1fU) >> 0U);
}
...
{
    return (uint32_t)(READ_BIT(RNGx->ISR, 0x1U) >> 15U);
}
