...
/** @addtogroup FM33LC0XX_FL_Driver_GPIO
  * @{
  */
...
#define         IS_FL_GPIO_PULL_UP(__VALUE__)               (((__VALUE__) == DISABLE)||\
                                                             ((__VALUE__) == ENABLE))
...
void FL_GPIO_StructInit(FL_GPIO_InitTypeDef *initStruct)
{
    ...
    initStruct->pull       = DISABLE;
    initStruct->remapPin   = DISABLE;
}
...
/**
  ...
  *			-PASS 配置成功  
  */
ErrorStatus FL_WKUP_DeInit(uint32_t wkupx)
{
    /* 入口参数检查 */
