...
/** @addtogroup FM33LC0xx_FL_Driver_DMA
  * @{
  */
...
#define         IS_FL_DMA_CIRC_MODE(__VALUE__)                      (((__VALUE__) == DISABLE)||\
                                                                     ((__VALUE__) == ENABLE))
...
/**
  ...
  *       	-FAIL 未成功执行
  */
ErrorStatus FL_DMA_DeInit(DMA_Type *DMAx)
{
...
/**
  ...
  *			-PASS 配置成功  
  */
ErrorStatus FL_DMA_Init(DMA_Type *DMAx, FL_DMA_InitTypeDef *initStruct, uint32_t channel)
{
    /* 参数检查 */
    assert_param(IS_FL_DMA_INSTANCE(DMAx));
...
/**
  ...
  *			-PASS 成功  
  */
ErrorStatus FL_DMA_StartTransmission(DMA_Type *DMAx, FL_DMA_ConfigTypeDef *configStruct, uint32_t channel)
{
    /* 配置传输个数 */
