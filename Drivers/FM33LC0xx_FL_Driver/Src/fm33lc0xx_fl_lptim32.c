...
/** @addtogroup FM33LC0xx_FL_Driver_LPTIM32
  * @{
  */

/* Private macros ------------------------------------------------------------*/
/** @addtogroup LPTIME_FL_Private_Macros
  * @{
  */
...
/**
  ...
  *       	-FAIL 未成功执行
  */
ErrorStatus FL_LPTIM32_DeInit(LPTIM32_Type *LPTIM32x)
{
...
/**
  * @brief	根据需要功能配置LPTIM32寄存器使之工作的定时器功能模式下 
  *         
  * @note 	其中 FL_LPTIM32_OPERATION_MODE_EXTERNAL_ASYNC_PULSE_CNT 模式需要外部脉冲提供给LPTIM32模块作为工作时钟，此时
  *      	LPTIM32完全工作在异步模式下。      
  * @param	LPTIM32x  外设入口地址
  * @param 	initStruct 指向 FL_LPTIM32_InitTypeDef 结构体，它包含指定LPTIM32外设的配置信息	  
  *         
  * @retval	ErrorStatus枚举值	
  *			-FAIL 配置过程发生错误        
  *			-PASS 配置成功        
  */
ErrorStatus FL_LPTIM32_Init(LPTIM32_Type *LPTIM32x, FL_LPTIM32_InitTypeDef *initStruct)
{
...
/**
  ...
  * @param 	initStruct 指向需要将值设置为默认配置的结构体 @ref FL_LPTIM32_InitTypeDef 结构体 
  *         
  * @retval	None
  */
...
/**
  * @brief	根据需要功能配置LPTIM32寄存器工作在输入捕获模式 
  *                
  * @param	LPTIM32x  外设入口地址
* @param 	initStruct_IC 为 @ref FL_LPTIM32_IC_InitTypeDef 结构体，它包含指定LPTIM32外设的配置信息	  
  * @param 	channel 指定LPTIM32的通道
  *
  * @retval	ErrorStatus枚举值	
  *			-FAIL 配置过程发生错误        
  *			-PASS 配置成功        
  */
ErrorStatus FL_LPTIM32_IC_Init(LPTIM32_Type *LPTIM32x, uint32_t channel, FL_LPTIM32_IC_InitTypeDef* initStruct_IC)
{
    ErrorStatus status = PASS;
...
/**
  * @brief	设置 initStruct_IC 为默认配置
  * @param 	initStruct_IC 指向需要将值设置为默认配置的结构体 @ref FL_LPTIM32_IC_InitTypeDef 结构体 
  *         
  * @retval	None
  */
...
/**
  ...
  *               
  * @param	LPTIM32x  外设入口地址
  * @param 	initStruct_OC 为 @ref FL_LPTIM32_OC_InitTypeDef 结构体，它包含指定LPTIM32外设的配置信息	  
  * @param 	channel 指定LPTIM32的通道
  *
  * @retval	ErrorStatus枚举值	
  *			-FAIL 配置过程发生错误        
  *			-PASS 配置成功        
  */
ErrorStatus FL_LPTIM32_OC_Init(LPTIM32_Type *LPTIM32x, uint32_t channel, FL_LPTIM32_OC_InitTypeDef *initStruct_OC)
{
...
/**
  * @brief	设置 initStruct_OC 为默认配置
  * @param 	initStruct_OC 指向需要将值设置为默认配置的结构体 @ref FL_LPTIM32_OC_InitTypeDef 结构体 
  *         
  * @retval	None
  */
