...
/** @addtogroup FM33LC0XX_FL_Driver_FLASH
  * @{
  */
...
ErrorStatus FL_FLASH_PageErase(FLASH_Type* FLASHx, uint32_t address)
{
    ...
    if(FL_FLASH_GetFlashLockStatus(FLASHx) != FL_FLASH_KEY_STATUS_SECTOR_ERASE)
    {/* Key 序列*/
...
/**
  ...
  *         -PASS 擦写成功  
  */   
ErrorStatus FL_FLASH_SectorErase(FLASH_Type* FLASHx, uint32_t address)
{
    uint32_t timeout = 0;
    uint32_t primask;
    ErrorStatus ret = PASS;
...
/**
  ...
  *         -PASS 擦写成功  
  */ 
ErrorStatus FL_FLASH_Program_Word(FLASH_Type* FLASHx, uint32_t address, uint32_t data)
{
    uint32_t timeout = 0;
    uint32_t primask;
    ErrorStatus ret = PASS;
...
/**
  ...
  *         -PASS 擦写成功  
  */ 
ErrorStatus FL_FLASH_Program_Sector(FLASH_Type* FLASHx, uint32_t sectorNum, uint32_t *data)
{
    uint32_t count;
    uint32_t primask;
    uint32_t address;
...
/**
  ...
  *         -PASS 擦写成功  
  */ 
ErrorStatus FL_FLASH_Write_Dma(FLASH_Type* FLASHx, uint32_t address, uint32_t *data)
{
    ErrorStatus ret; 
    uint32_t primask;
    uint32_t timeout;
